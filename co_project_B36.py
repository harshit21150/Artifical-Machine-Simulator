...
def codelen4(a,b,c,d,linenum):
    ...
    if a not in validfunclist:
        ...
        if check==False:
            resstring="ERROR:"+str(linenum)+"INVALID REGISTER TYPE"
        else:
...
def codelen1(a):
    ...
    if a not in funclist:
        print("improper function implemented")
    else:
...
def typeb(a,b,c,linenum):
    ...
    if a not in funclist:
        ...
        if (c[1:]).isdigit():
            ...
            resstring="ERROR: improper immediate value entered by the user"
    return resstring
...
################################################
    ...
    for i in range(len(varlist),len(code)):
        ...
        if len(spacecheck2)==0:
            ...
            resstring="ERROR: INVALID INSTRUCTION"
            ...
            if code[i][2] in varlist:
                ...
                resstring="ERROR: INVALID INSTRUCTION IMPLIED"
        elif codelen==2:
            if code[i][0]=="var":
                ...
                resstring="ERROR: IMPROPER SYNATX/INSTRUCTION IN LINE "+str(i)
        elif codelen==1:
            if code[i][0]=="hlt":
                if i+1==len(code):
                    resstring=codelen1(code[i][0])
                else:
                    resstring="ERROR: hlt USED IN BETWEEN THE PROGRAMME"
            else:
                resstring="ERROR: INVALID INSTRUCTION"
