...
def codelen4(a,b,c,d,linenum):
    ...
    if a not in validfunclist:
        resstring="ERROR: IN VALID/ INCOMPLETE INSTRUCTION"
...
#########################################
    ...
    for i in range(0,len(code)):
        ...
        if len(spacecheck)==0:
            ...
            sublen=len(code[i])
            if sublen>5:
                labresstring="ERROR: INVALID OPERATION IN GIVEN :"+str(code[i][0])+"in line "+str(i+1)
                ...
                if code[i][3][0]=="$" and (code[i][3][1:]).isdigit():
                    ...
                    resstring="ERROR: REPEATING LABEL / NOT A VALID REGISTER TYPE/OR A IMMEDIATE VALUE"
                    labelresult[code[i][0]]=resstring
    '''print(labelpc)
    print(labelresult)
    print(varlist)
